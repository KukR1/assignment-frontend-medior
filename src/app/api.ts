...
export const api = createApi({
  ...
  endpoints: (builder) => ({
    ...
    deleteFruit: builder.mutation<string, string>({
      queryFn: (fruitId) =>
        resolveAsync({ data: fruitsCrud.deleteItem(fruitId) }),
      invalidatesTags: ['Fruit'],
    }),
    ...
    addVegetable: builder.mutation<Vegetable, Omit<Vegetable, 'id'>>({
      queryFn: (vegetable) =>
        resolveAsync({ data: vegetablesCrud.addItem(vegetable) }),
    ...
    deleteVegetable: builder.mutation<string, string>({
      queryFn: (vegetableId) =>
        resolveAsync({ data: vegetablesCrud.deleteItem(vegetableId) }),
      invalidatesTags: ['Vegetable'],
    }),
